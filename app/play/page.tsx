    ...
    <div style={{ background: "url('/assets/DungeonBackground.png') center center / cover no-repeat, linear-gradient(to bottom, rgba(0,0,0,0.8), rgba(88,28,135,0.6))" }} className="min-h-screen bg-gradient-to-b from-black to-purple-900/30 pt-24 pb-12 px-4">
      <div className="container mx-auto max-w-6xl">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 justify-center items-center max-w-3xl mx-auto">
            {gameModes.map((mode) => (
              <div
                key={mode.id}
                className={`bg-black/60 rounded-2xl p-8 w-full flex-1 transition-colors cursor-pointer border-4 flex flex-col items-center hover:bg-gray-800 shadow-xl backdrop-blur-sm ${selectedGameMode === mode.id ? 'border-purple-700/40 bg-purple-900/40' : 'border-transparent'}`}
        ...
        <div className="flex justify-center mt-12">
          <button
            className={`px-10 py-4 rounded-xl text-lg font-bold transition-colors ${selectedChampion && selectedGameMode ? 'bg-purple-600 hover:bg-purple-700 cursor-pointer' : 'bg-gray-700 cursor-not-allowed'} fantasy-title text-2xl text-purple-300 drop-shadow-lg hover:text-yellow-300`}
