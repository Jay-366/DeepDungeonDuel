...
const Bet: React.FC<BetProps> = ({ userAgent, enemyAgent, walletAddress, balance, onBattle }) => {
  ...
  return (
    <div style={{ background: "url('/assets/DungeonBackground.png') center center / cover no-repeat, linear-gradient(to bottom, rgba(0,0,0,0.8), rgba(88,28,135,0.6))" }} className="min-h-screen w-full flex items-center justify-center">
      <div className="w-full max-w-3xl mx-auto bg-black/60 backdrop-blur-sm rounded-2xl shadow-2xl p-8 flex flex-col items-center border-4 border-purple-700/40">
        {/* Wallet Card */}
        ...
        <div className="flex w-full justify-between items-center my-8 gap-8">
          {/* User Agent */}
          <div
            className={`flex-1 bg-black/60 rounded-xl p-6 flex flex-col items-center border-4 transition-all cursor-pointer shadow-xl backdrop-blur-sm ${selectedAgent === userAgent.id ? 'border-purple-500 shadow-lg' : 'border-transparent'}`}
          ...
          >
            ...
            <h3 className="text-lg font-bold text-white mb-1 fantasy-title">{userAgent.name}</h3>
          ...
          {/* Enemy Agent */}
          <div
            className={`flex-1 bg-black/60 rounded-xl p-6 flex flex-col items-center border-4 transition-all cursor-pointer shadow-xl backdrop-blur-sm ${selectedAgent === enemyAgent.id ? 'border-purple-500 shadow-lg' : 'border-transparent'}`}
          ...
          >
            ...
            <h3 className="text-lg font-bold text-white mb-1 fantasy-title">{enemyAgent.name}</h3>
        ...
        <div className="w-full max-w-md mx-auto bg-black/60 rounded-xl p-6 flex flex-col items-center mb-6 border-4 border-purple-700/40 backdrop-blur-sm">
          <div className="flex items-center mb-4 w-full justify-between">
            <span className="text-white font-bold text-lg fantasy-title">Bet Amount (SOL)</span>
        ...
        {/* Battle Button */}
        <button
          className={`w-full max-w-md mx-auto px-8 py-4 rounded-xl text-lg font-bold transition-colors fantasy-title text-2xl text-purple-300 drop-shadow-lg hover:text-yellow-300 ${selectedAgent && betAmount ? 'bg-green-600 hover:bg-green-700 text-white cursor-pointer' : 'bg-gray-700 text-gray-400 cursor-not-allowed'}`}
